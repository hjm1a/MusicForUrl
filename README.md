...
生成链接时提供两种输出：
- **HLS（转码分片）**：兼容性最好（**VRChat 推荐**），但会消耗较多 CPU/磁盘（需要 FFmpeg）。
- **轻量 M3U8（直链列表）**：几乎不转码/不落盘，服务器压力更小；但它不是标准的 TS/fMP4 分片流，兼容性取决于播放器实现（在部分环境含部分 VRChat 播放器可用，但不保证）。如遇不播放，请切换到 HLS。
